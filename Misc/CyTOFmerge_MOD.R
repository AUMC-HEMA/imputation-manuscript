# MODIFIED VERSION
# Function is changed so that if only one marker is imputed, it does not raise an error
# and it calculates the median appropriately.
# Warning: hard coded for this specific purpose only!

CombineFCS_MOD <- function (FCSfile1,RelevantMarkers1,FCSfile2,RelevantMarkers2,arcsinhTrans=FALSE)
{
  ...
  for (i in c(1:dim(FCS1.data)[1])){
    ...
    Data.combine.1[i,(dim(FCS1.data)[2]+1):dim(Data.combine.1)[2]] = as.matrix(median(FCS2.data[IDX1[i,],(m+1):dim(FCS2.data)[2]]))
  }
