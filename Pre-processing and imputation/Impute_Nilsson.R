...
ff1 <- read.FCS(paste0(export_dir, 'Nilsson_rare', '_ff1.fcs'))
ff2 <- read.FCS(paste0(export_dir, 'Nilsson_rare', '_ff2.fcs'))
gt <- read.FCS(paste0(export_dir, 'Nilsson_rare', '_gt.fcs'))
...
write.csv(merged$metadata, paste0(export_dir, 'Nilsson_rare', '_CBB_meta.csv'))
write.csv(merged$FCS1_excluded, paste0(export_dir, 'Nilsson_rare', '_FCS1_exclusions.csv'))
write.csv(merged$FCS2_excluded, paste0(export_dir, 'Nilsson_rare', '_FCS2_exclusions.csv'))
...
colnames(cyCombine_exprs) <- gsub("PE.Texas.Red.A", "PE-Texas Red-A", colnames(cyCombine_exprs), fixed=TRUE)
colnames(cyCombine_exprs) <- gsub("Alexa.Fluor.700.A", "Alexa Fluor 700-A", colnames(cyCombine_exprs), fixed=TRUE)
colnames(cyCombine_exprs) <- gsub("Qdot.605.A", "Qdot 605-A", colnames(cyCombine_exprs), fixed=TRUE)
colnames(cyCombine_exprs) <- gsub("Qdot.655.A", "Qdot 655-A", colnames(cyCombine_exprs), fixed=TRUE)
colnames(cyCombine_exprs) <- gsub("QDot.800.A", "QDot 800-A", colnames(cyCombine_exprs), fixed=TRUE)
...
infinicyt_exprs <- read.csv(paste0(export_dir, 'Nilsson_rare_infinicyt.txt'), sep=';')
# Rename columns
# Get the names from previous step
names <- colnames(cyCombine_exprs)[-11]
...
for (method in names(imputed_ffs)){
  ...
  write.FCS(combined, paste0(export_dir, 'Nilsson_rare', '_', method, '_exprs.fcs'))
}
